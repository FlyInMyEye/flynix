  ...
  home-manager.sharedModules = [
    ({ config, ... }: {
      ...
      mainBar = { 
        ...
        modules-right = [ "network" "bluetooth" "hyprland/language" "custom/battery" "pulseaudio" "clock" "custom/lock" "custom/power" "tray"];
        ...
        "hyprland/workspaces" = {
          ...
          format = "{name}";
          on-click = "activate";
          # Fix workspace freezing by disabling persistent workspaces
          # This forces waybar to only show active workspaces from Hyprland
          active-only = false;
          sort-by-number = true;
        };
        ...
        "custom/battery" = {
          exec = ''
            ...
            ultra_mode_active=$([ -f /tmp/ultra-battery-mode-active ] && echo "yes" || echo "no")
            ...
            if [ "$status" = "Charging" ]; then
              echo "󱐋 $capacity%"
            elif [ "$ultra_mode_active" = "yes" ]; then
              echo "<span color='#FFA500'>󰌪 </span><span color='#28CD41'>$icon  </span>$capacity%"
            else
              echo "<span color='#28CD41'>$icon  </span>$capacity%"
            fi
          '';
          format = "{}";
          interval = 2;
          on-click = "${config.home.homeDirectory}/.local/bin/toggle-battery-mode.sh";
          tooltip = true;
        };
